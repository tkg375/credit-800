...
export async function sendIssueReport(params: { userId: string; userEmail: string; fullName?: string; plan?: string; subscriptionStatus?: string; issue: string; page?: string }): Promise<void> {
  const { userId, userEmail, fullName, plan, subscriptionStatus, issue, page } = params;
  await sendEmail(
    "tgordo03@gmail.com",
    ...
    `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#1e293b">
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;font-size:13px">
        <tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 0;color:#64748b;width:130px">Name</td><td style="padding:10px 0;font-weight:600">${escHtml(fullName || "—")}</td></tr>
        <tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 0;color:#64748b">Email</td><td style="padding:10px 0">${escHtml(userEmail)}</td></tr>
        <tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 0;color:#64748b">Account ID</td><td style="padding:10px 0;font-family:monospace;font-size:12px">${escHtml(userId)}</td></tr>
        <tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 0;color:#64748b">Plan</td><td style="padding:10px 0;text-transform:capitalize">${escHtml(plan || "free")} · ${escHtml(subscriptionStatus || "—")}</td></tr>
        ${page ? `<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 0;color:#64748b">Page</td><td style="padding:10px 0">${escHtml(page)}</td></tr>` : ""}
        <tr><td style="padding:10px 0;color:#64748b">Reported At</td><td style="padding:10px 0">${new Date().toLocaleString("en-US", { timeZone: "America/New_York" })} ET</td></tr>
      </table>
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:20px;margin-bottom:24px">
        <p style="margin:0 0 10px;font-weight:600;font-size:14px;color:#0f172a">Issue Description</p>
        <p style="margin:0;font-size:14px;color:#334155;white-space:pre-wrap;line-height:1.6">${escHtml(issue)}</p>
      </div>
